...
fft_freq_d = np.fft.rfftfreq(1024, d=1.0/6250e3)*2
...
def vis_thread(frame_queue, message_queue, runflag):
    ...
    while runflag.value == 1:
        ...
        try:
            cmd, xs, ys, zs, peaks, ranges = frame_queue.get(
        ...
        for i in range(len(xs)):
            ...
            range_idx = ranges[i]
            if range_idx >= fft_distance.shape[0] or fft_distance[range_idx] < 0.4:
                continue
            r = int(500/range_idx)
            # r = 5

            xx = int((angle_h/FoVrh + 1) * 0.5*w + of)
            zz = int((angle_v/FoVrv + 1) * 0.5*h)

            n = y/2*255

            color = (0, 255, 255)
            cv2.circle(frame, (xx, zz), r, color, -1)

            if mode == 'run':
                class_idx = labels[i]
                if class_idx == -1:
                    cv2.circle(obj, (xx, zz), r, (0, 255, 255), -1)
                else:
                    cv2.circle(obj, (xx, zz), r, colors[class_idx], -1)
